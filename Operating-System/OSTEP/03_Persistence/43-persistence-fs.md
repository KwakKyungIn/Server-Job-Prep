...
### 43.7 캐싱과 버퍼링 (Caching and Buffering)

디스크 I/O는 매우 느리기 때문에, 대부분의 운영체제는 **캐시 계층(cache layer)**을 사용하여 성능을 향상시킨다.

#### 🧠 목적

- 자주 사용하는 블록을 메모리에 유지 → 디스크 접근 최소화
- **쓰기 지연(write-back)** 혹은 **쓰기 즉시(write-through)** 정책 선택 가능

#### 📦 캐시 구조

| 구성 요소 | 설명 |
|-----------|------|
| 블록 캐시 | 최근 사용한 데이터 블록들을 메모리에 저장 |
| inode 캐시 | 최근 접근한 inode 메타데이터를 보관 |
| 디렉터리 캐시 | 경로 탐색 시 반복되는 디렉터리 검색을 줄임 |

#### 💥 동기화 문제

- 여러 프로세스가 같은 파일을 동시에 접근할 경우 **캐시 일관성 문제 발생**
- `fsync()` 또는 저널링(journaling) 등을 통해 일관성을 보장할 수 있음

---

### 43.8 요약 (Summary)

이 장에서는 간단한 파일 시스템 구현을 통해  
**파일 → inode → 데이터 블록**의 구조를 이해하고,  
파일 시스템의 내부 동작과 구현 전략을 살펴봤다.
...
|-----------|------|
| 슈퍼블록 | 전체 파일 시스템 정보 |
| inode | 파일 메타데이터 및 데이터 위치 정보 |
| 디렉터리 | 이름 ↔ inode 번호 매핑 테이블 |
| 데이터 블록 | 실제 사용자 데이터 저장 공간 |
| 빈 공간 관리 | 비트맵, 자유 리스트 등으로 블록 추적 |
| 읽기/쓰기 흐름 | 블록/메타데이터 접근 순서 중요 |
| 캐시 계층 | 성능 향상을 위한 메모리 내 저장소 |

> 실전 파일 시스템은 여기서 배운 기초 위에 다양한 고급 기법(트랜잭션, 저널링, 확장성, 복구 등)을 덧붙여 구현된다.
...
| 항목 | 설명 |
|------|------|
| 파일 ↔ inode | 파일 이름은 디렉터리에, 실체는 inode에 저장됨 |
| inode ↔ 데이터 블록 | 포인터 구조 (직접, 간접 등)를 통해 연결 |
| 디스크 구성 | [부트 | 슈퍼블록 | inode 배열 | 데이터 블록] |
| 빈 공간 관리 | 비트맵 or 자유 리스트 |
| I/O 흐름 | open → inode 탐색 → 블록 접근 |
| 성능 향상 | 캐싱, 버퍼링, 순서 보장, fsync 등 |

---

> 📚 출처: 『운영체제: 아주 쉬운 세 가지 이야기 (OSTEP)』 Chapter 43  
