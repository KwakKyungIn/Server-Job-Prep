...
void GameRoom::EnterProjectile(ProjectileRef p)
{
    ...
    // viewers 계산(몬스터 EnterMonster와 동일)
    Vector<Zone*> zones;
...
void GameRoom::LeaveProjectile(uint64 projectileId)
{
    ...
    // viewers에게 despawn + player visible set 정리
    {
...
void GameRoom::UpdateProjectiles(uint64 deltaMs)
{
    ...
    for (auto& kv : _projectiles)
    {
        ...
        // 스킬 데이터 (damage / stopOnHit / hitRadius / maxHits)
        ...
        // owner 찾기 (playerId or monster objectId)
        ...
        // 후보 존 수집: oldZone + newZone 주변 union
        ...
        // connectivity (투사체는 “impact 위치” 기준으로도 체크할 건데,
        // 1차는 newPos 기준 coarse filter로 충분)
        ...
        if (!hits.empty() && p->CanHitMore())
        {
            ...
            for (const HitCand& h : hits)
            {
                ...
                // impact pos로 투사체 위치 스냅(비주얼 과관통 방지)
                ...
                // stopOnHit or maxHits 도달 → despawn
