...
enum : uint16
{
	...
	PKT_C_RECONNECT_REQ = 1002,
	PKT_S_RECONNECT_RES = 1003,
	PKT_C_PRIVATE_CHAT_REQ = 1004,
	PKT_S_PRIVATE_CHAT_NTF = 1005,
	PKT_C_CREATE_ROOM_REQ = 1006,
...
public:
	static void Init()
	{
		...
		GPacketHandler[PKT_S_RECONNECT_RES] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<Protocol::S_RECONNECT_RES>(Handle_S_RECONNECT_RES, session, buffer, len); };
	...
	static SendBufferRef MakeSendBuffer(Protocol::C_RECONNECT_REQ& pkt) { return MakeSendBuffer(pkt, PKT_C_RECONNECT_REQ); }
	static SendBufferRef MakeSendBuffer(Protocol::C_PRIVATE_CHAT_REQ& pkt) { return MakeSendBuffer(pkt, PKT_C_PRIVATE_CHAT_REQ); }
	...
	static SendBufferRef MakeSendBuffer(Protocol::C_FRIEND_ADD_REQ& pkt) { return MakeSendBuffer(pkt, PKT_C_FRIEND_ADD_REQ); }
	static SendBufferRef MakeSendBuffer(Protocol::C_FRIEND_LIST_REQ& pkt) { return MakeSendBuffer(pkt, PKT_C_FRIEND_LIST_REQ); }
