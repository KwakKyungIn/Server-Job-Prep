...
> **vtable (Virtual Table)** 은 C++에서 **런타임 다형성(dynamic polymorphism)** 을 구현하기 위한 핵심 메커니즘이다.
>
> 가상 함수(`virtual function`)가 있는 클래스는 컴파일러가 자동으로 **가상 함수 테이블(vtable)** 과
> **vtable 포인터(vptr)** 를 생성하여,
> 실행 시점에 **실제 객체 타입에 맞는 함수를 호출**할 수 있게 한다.
...
| 구성 요소                  | 설명                                                     |
| ---------------------- | ------------------------------------------------------ |
| **vtable (가상 함수 테이블)** | 클래스 단위로 존재하는 함수 포인터 배열                                 |
| **vptr (가상 테이블 포인터)**  | 객체마다 존재하며, 자신이 속한 클래스의 vtable을 가리킴                     |
| **목적**                 | 정적 바인딩(static binding)을 런타임 바인딩(dynamic binding)으로 바꿔줌 |
...
1. **컴파일러가 vtable 생성**
   ...
   * Base → vtable(Base::Speak)
   * Derived → vtable(Derived::Speak)

2. **객체 생성 시**

   * `Base`의 생성자에서 vptr 설정
   * `Derived` 생성 시 vptr이 Derived의 vtable로 갱신됨

3. **호출 시**

   * `ptr->vptr`을 따라가서 Derived의 vtable 참조
   * vtable[0] → `&Derived::Speak`
   * 해당 주소의 함수를 호출
...
[객체 메모리 구조]
 ...
 │ 멤버 변수들       │         ├── &Derived::Speak
 │                   │         ├── &Derived::Attack
 └────────────────────┘         └── ...
```

➡️ vptr은 “이 객체의 진짜 타입”을 가리키고,
➡️ vtable은 “그 타입이 어떤 가상 함수를 실제로 사용하느냐”를 알려주는 테이블이다.
...
즉,
1️⃣ `ptr->vptr` → 해당 객체의 vtable 주소
2️⃣ `[0]` → 첫 번째 가상 함수 포인터
3️⃣ 함수 포인터를 역참조 후 호출

➡️ 최종적으로 **실제 객체 타입의 함수(파생 클래스)** 가 호출된다.
...
| 시점         | 동작                                             |
| ---------- | ---------------------------------------------- |
| **생성자 내부** | vptr이 아직 완전히 설정되지 않음 → 자기 클래스의 vtable만 사용      |
| **소멸자 내부** | vptr이 기본 클래스의 vtable로 되돌아감 → 파생 클래스 가상 함수 호출 X |

📌 즉, **생성자나 소멸자에서 가상 함수를 호출하면 base 버전만 호출됨.**
...
* C는 **A용 vtable**, **B용 vtable** 두 개를 가짐
* 객체 내부에도 **vptr이 2개 존재** (각 base용)
...
> 다중 상속에서는 포인터 변환 시 **vptr 오프셋 조정(pointer adjustment)** 이 필요하다.
...
| 객체당  | vptr 1개 (보통 8바이트)       |
| 클래스당 | vtable 1개 생성            |
| 호출 시 | 포인터 간접 참조 1회 (약간의 오버헤드) |

➡️ 하지만 대부분의 경우 최적화로 무시 가능한 수준이다.
...
| 상황                  | 최적화 여부                |
| ------------------- | --------------------- |
| **정적 타입이 확정된 경우**   | 인라인 최적화 가능            |
| **가상 함수 호출 빈번한 경우** | 분기 예측 실패 시 성능 저하 가능   |
| **가상 소멸자 필수 클래스**   | 객체 파괴 시 정확한 소멸자 호출 보장 |

---

## 🎯 **면접용 정리**

> vtable은 **클래스 단위의 함수 포인터 테이블**,
> vptr은 **객체 내부에서 해당 테이블을 가리키는 포인터**입니다.
>
> 가상 함수 호출 시 **컴파일러가 vptr을 통해 vtable을 참조**하여
> **실제 객체 타입에 맞는 함수 주소를 찾아 호출**함으로써
> **런타임 다형성(dynamic dispatch)** 을 구현합니다.

---

### 🔥 꼬리질문 예상

1. **Q. 가상 함수 호출의 오버헤드는 어느 정도인가요?**
   → 함수 포인터 한 번 역참조하는 수준으로, 일반 함수 호출보다 약간 느림.

2. **Q. 생성자에서 가상 함수를 호출하면 어떤 함수가 실행되나요?**
   → 현재 클래스의 버전만 호출됨 (파생 클래스 함수는 호출되지 않음).

3. **Q. 다중 상속 시 vtable은 몇 개 생성되나요?**
   → 상속받은 각 base class마다 별도의 vtable이 존재.

4. **Q. vtable은 프로그램 어디에 저장되나요?**
   → 전역 상수 데이터 영역(`.rdata`)에 클래스당 하나씩 저장된다.
